const namaInput = document.getElementById('nme') // ID: nme
const estimasiInput = document.getElementById('estimasi') // ID: estimasi
const tanggalInput = document.getElementById('tanggal') // ID: tanggal
const alasanInput = document.getElementById('rson') // ID: rson
...
form.addEventListener('submit', async (e) => {
  ...
  try {
    const response = await fetch('http://localhost:3000/api/izin', {
      method: 'POST',
      headers: {
    ...
    form.reset();
    [namaInput, estimasiInput, tanggalInput, alasanInput].forEach((el) => {
      el.value = ''
    })
  }
})
